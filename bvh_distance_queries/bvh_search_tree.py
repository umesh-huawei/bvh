...
class BVH(nn.Module):
    ...
    def forward(
        ...
        output = BVHFunction.apply(triangles, points)
...
class BVHRayIntersectionFunction(autograd.Function):
    ''' Autograd wrapper for the BVH nearest neighbor kernel
    '''
    QUEUE_SIZE = 128
    SORT_POINTS_BY_MORTON = True
    ...
    def forward(ctx,
                triangles: Tensor,
                ro: Tensor, rd: Tensor):
        outputs = bvh_distance_queries_cuda.ray_queries(
            triangles, ro, rd,
            queue_size=BVHFunction.QUEUE_SIZE
        )
        ctx.save_for_backward(triangles, *outputs)
...
class BVHRayIntersection(nn.Module):

    def __init__(self,
                 queue_size: int = 128) -> None:

        super(BVHRayIntersection, self).__init__()
        assert queue_size in [32, 64, 128, 256, 512, 1024], (
            f'Queue/Stack size must be in {str[32, 64, 128, 256, 512, 1024]()}'
        )
        BVHRayIntersectionFunction.QUEUE_SIZE = queue_size
    ...
    def forward(
        ...
        output = BVHRayIntersectionFunction.apply(triangles, ro, rd)
        
        distances, closest_points, closest_faces = output
