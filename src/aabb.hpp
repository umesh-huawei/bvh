...
__host__ __device__ vec2<T> ray_intersect(const vec3<T>& pos, const vec3<T>& dir) const {
		T tmin = (min_t.x - pos.x) / dir.x;
		T tmax = (max_t.x - pos.x) / dir.x;

		if (tmin > tmax) {
			host_device_swap(tmin, tmax);
		}

		T tymin = (min_t.y - pos.y) / dir.y;
		T tymax = (max_t.y - pos.y) / dir.y;
		...
		if (tmin > tymax || tymin > tmax) {
			return make_vec2(std::numeric_limits<T>::max(), std::numeric_limits<T>::max());
		}
		...
		T tzmin = (min_t.z - pos.z) / dir.z;
		T tzmax = (max_t.z - pos.z) / dir.z;
